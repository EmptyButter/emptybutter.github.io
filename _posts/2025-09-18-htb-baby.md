...
There are various tools for password spraying. I like using `kerbrute` as it leverages Kerberos pre-authentication, which is extremely fast. However, mind that it still increments failed password attempt counters, which would lead to unintended account lockouts if not being careful.
```
